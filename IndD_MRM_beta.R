...
library(doParallel)
set.seed(123)
# load metadata & beta diversity
metadata<- read.delim('2.tsv', sep = '\t',row.names = 1)%>%
  mutate(X.SampleID = rownames(.))#read metadata, delete rowname
...
#################
#Sampling details distance(technical factors)
...
f = 'm$beta ~ m$diet + m$habitat + m$geo + m$samp'
...
plot_write(p, file='IndD_MRM_beta.pdf')
